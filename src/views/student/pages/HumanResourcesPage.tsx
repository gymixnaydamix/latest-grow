...
export default function HumanResourcesPage() {
  ...
  const [formType, setFormType] = useState<HRRequest['type']>('enrollment');
  ...
  const handleSubmit = () => {
    ...
    submitMut.mutate(
      { title: formTitle.trim(), category: formType, description: formDesc.trim() },
      {
        onSuccess: () => {
          notifySuccess('Request', 'HR request submitted');
          setFormTitle(''); setFormType('enrollment'); setFormDesc('');
          setFormErrors({}); setShowNew(false);
        },
        onError: () => notifyError('Error', 'Failed to submit HR request'),
      },
    );
  };
          ...
          <CardContent className="flex flex-col gap-3">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <select value={formType} onChange={(e) => setFormType(e.target.value as HRRequest['type'])} className="h-8 rounded-md border border-white/8 bg-white/4 px-3 text-xs text-white/60 outline-none">
            ...
            <div className="flex flex-col gap-1">
              ...
              <textarea value={formDesc} onChange={(e) => { setFormDesc(e.target.value); setFormErrors((p) => ({ ...p, description: '' })); }} rows={3} placeholder="Describe your HR request in detail…" className={cn('rounded-lg border border-white/8 bg-white/4 px-3 py-2 text-xs text-white/80 placeholder:text-white/25 outline-none resize-none focus:border-blue-400/40', formErrors.description && 'border-red-400/50')} />
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button size="sm" onClick={handleSubmit} disabled={submitMut.isPending} className="text-xs bg-blue-500/20 text-blue-300 border border-blue-400/20 gap-1"><Send className="size-3" />{submitMut.isPending ? 'Submitting…' : 'Submit'}</Button>
            </div>
          </CardContent>
        </Card>
      )}
